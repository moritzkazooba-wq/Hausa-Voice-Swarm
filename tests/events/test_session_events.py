...
def mock_producer() -> KafkaEventProducer:
    """Create a KafkaEventProducer with mocked publish."""
...
class TestSessionEventHelpers:
    async def test_emit_intent_classified(self, mock_producer: KafkaEventProducer) -> None:
        ...
        mock_producer.publish.assert_called_once()  # type: ignore[union-attr]
        event = mock_producer.publish.call_args[0][0]  # type: ignore[union-attr]
        assert event.event_type == "intent_classified"
        assert event.session_id == "sess-1"
        assert event.intent == "balance_check"

    async def test_emit_tool_executed(self, mock_producer: KafkaEventProducer) -> None:
    ...
    async def test_emit_session_started(self, mock_producer: KafkaEventProducer) -> None:
    ...
    async def test_emit_session_ended(self, mock_producer: KafkaEventProducer) -> None:
    ...
    async def test_emit_escalation(self, mock_producer: KafkaEventProducer) -> None:
